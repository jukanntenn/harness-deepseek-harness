...
description: Manually run the extended hdsh bilingual-document workflow, including generated briefings, delegated prose translation, whole-document translation, and scoped pairing verification.
...
Run this extended workflow only when the user explicitly invokes `translating-docs` by name. Never select or load it for ordinary documentation work, from another skill, or from an inferred translation need; routine translation follows the one-shot, one-pass rule in [docs/AGENTS.md](../../../docs/AGENTS.md).
...
Frozen RFCs under `.agents/rfcs/archived/` are not translation work. Their complete triplets are sealed by the archive gate; never update, re-record, or repair either side after archival.

## The update path (briefing-driven)

The briefing-driven path matches guidance-corpus quality at a fraction of the cost; the [briefed-updates RFC](../../rfcs/implemented/process/2026-09-07-briefed-minimal-translation-updates.md) owns the rationale.

1. **Generate the briefing**: `uv run hdsh pairing brief <any file of the pair>` (no arguments briefs every out-of-sync pair). The briefing maps the change at the narrowest safely aligned granularity — changed Markdown units (paragraph, table row, list item, heading), then whole heading sections, then whole document — and contains the authored side's diff since the last confirmed-consistent state, each changed unit's last-confirmed source, current source, and current counterpart text (with line numbers), the terminology rows the change touches, first-occurrence movement notes, and a digest of the binding update rules.
2. **Mechanical-only diff? `--apply` it.** When every change lies inside code fences that the pair shares byte-identically, the briefing says so; `uv run hdsh pairing brief --apply <pair>` splices the edited fences into the counterpart and structure-validates the result before writing — no subagent, no hand-editing.
...
5. **Record and verify, scoped**: `uv run hdsh pairing record <pair>` then `uv run hdsh pairing verify <pair>`. Recording names exactly the pairs you confirmed — a bare record is rejected so a bulk re-record is always an explicit `--all`. The corpus-wide check still runs in CI; do not run it per-update.
...
- **[editing-prose](../editing-prose/SKILL.md)** — required prose coverage and editorial judgment. Apply it to both sides without adding or dropping source propositions.
...
- Code blocks are byte-identical across the pair, comments included. Repository-relative document links keep the same semantic target and exact query/fragment suffix: targets in the active bilingual corpus use `.md` on the English side and `.zh.md` on the Chinese side, a missing in-scope counterpart is an error, targets outside the corpus keep the authored path, and the switcher remains the cross-locale exception.
...
- `uv run hdsh pairing list` prints every in-scope document as missing / out-of-sync / ok. Missing and out-of-sync rows are contract violations; the normal check rejects them.
- `uv run hdsh pairing brief` with no arguments prints the briefing for every out-of-sync pair.
...
1. Switcher: `[English](foo.md) | 中文` immediately after the Chinese file's H1, `English | [中文](foo.zh.md)` after the English file's H1 — add both if this is a new pair, except that a manifest-`generated` English source stays byte-identical to generator output and omits its switcher while the Chinese counterpart still links back.
2. Record consistency: `uv run hdsh pairing record <pair>` recomputes and records both sides' full blob hashes in `foo.i18n.yaml`. The yaml diff in your PR is the reviewable statement "I confirmed these two say the same thing" — only run it after you actually have.
3. No manifest entry is needed for an ordinary document: every in-scope source requires a pair. Change [.hdsh/pairing.manifest.json](../../../.hdsh/pairing.manifest.json) only when the owning policy documents a genuine generated, instructional, or bilingual-by-construction exclusion.
4. Before the PR: the touched pairs are green under the scoped check; the prek hooks run on every commit and the corpus-wide pairing check runs once at PR level per [pushing](../pushing/SKILL.md), not inside each translation task.
...
Follow the [code-review reporting guidance](../reviewing/SKILL.md#reporting-findings): evaluate each comment on its merits, and for terminology comments, remember the terminology table is the contract — apply a reviewer's rendering decision to [terminology.md](../../../docs/i18n/terminology.md), not only to one file.
