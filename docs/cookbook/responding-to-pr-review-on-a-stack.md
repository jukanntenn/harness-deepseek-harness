...
Review comments may target several PRs in a dependent stack (`A ← B ← C …`). Keep that chain linked through GitHub's official stacked-PR feature. This guide owns review-fix placement and propagation; the [merging-stacked-prs](../../.agents/skills/merging-stacked-prs/SKILL.md) skill owns linkage checks and landing.

## Ground rules

1. **One worktree per PR branch.** Each PR's fixes happen in that PR's own worktree; parallel fixes never share a checkout. Hooks are worktree-local, so install them in each new worktree with `uv run hdsh worktree install`.
...
3. Propagate the fixed layer through every affected child in order:
   - **Merge-forward:** merge the fixed parent branch into its child, validate the child, and continue upward. Preserve each in-progress checkpoint under the [local Git workflow RFC](../../.agents/rfcs/implemented/process/2026-09-07-local-git-workflow.md).
   - **Native cascading rebase:** use `gh stack rebase`, validate the rewritten layers, then publish with `gh stack push`; or use `gh stack sync`, which may publish first and therefore requires immediate post-sync validation under [pushing](../../.agents/skills/pushing/SKILL.md).
4. Treat delegated fixes as trust-but-verify: a subagent's report describes intent, not necessarily what landed. Re-run the gates yourself on the actual tree, and for a regression guard, prove it FAILS on the unfixed code (introduce the regression, watch red, revert) — a guard that passes both ways guards nothing. A subagent that reframes a problem as already handled is a signal to dig in personally.
