...
Use them to identify the governing principle, not as text templates. This file deliberately quotes leaked wording as calibration material — the [recall batteries](recall-batteries.md) exclude the skill's directory, and its wording is not a license elsewhere.
...
**Fixed:** "Slash input resolves against the visible catalog — the plain-text-reference decision, owned by the input-machine RFC."

The ordinal resolves nowhere at HEAD; the decision's name and owning RFC path do. Name the owning RFC's path at least once per file — as a link where the surface supports one — and later mentions may use the searchable name alone.
...
Both live facts survive — the current mechanism and the standing failure behavior — restated in the present. The bug's biography belongs to the PR and its RFC.
...
"This cut" / "v1" / "today" go stale the moment they merge. A historical stage name inside an RFC's change-story section ("the first cut shipped X") is current-state-safe; the indexical form never is.
...
**Fixed (in an RFC's Alternatives considered):** "**Caching the resolved spec.** Rejected: the spec depends on per-call cwd, so a cache keyed by request would serve stale roots."

The alternatives-considered genre is the sanctioned home; the reviewer and the round are not part of the rationale.

### Draft ordinals

**Leaked:** "As of v5 of this RFC, the loader also validates manifests."

**Fixed:** "The loader validates manifests."

An implemented RFC states shipped reality; its own revision history lives in git.
...
**Fixed:** (deleted; the wrapping order is stated in the function's docstring.)
...
**Fixed:** "The renderer runs on the client side; spacing follows the published design spec."

Working-language fragments and session separators are transcription residue. An external provenance reference that resolves outside the repo by design stays.
...
**Suspect:** A CLI diagnostic says "available for now", and its owning test pins the exact string.

**Wrong:** Rewrite the string during a prose-only purge without touching the test, or weaken the test to match the new wording.

**Right:** Route the wording through the owning behavior: update the diagnostic and its pinning test in the same change, carried by the behavior evidence that justifies the new wording. If the authorized scope has no owning test, leave the wording unchanged and report the deferral.
...
An unaided pass deleted this, reasoning that issue citations belong in RFCs. Wrong direction: issues resolve at HEAD from any surface, and "#N owns the follow-up" is the sanctioned home for deferred work in a README. What RFCs additionally sanction is citing *merged PRs* as evidence.
...
**Keep (after fixing):** `# pragma: no cover  # the one-element literal guarantees index 0.`
...
Every trap below shipped in an original purge and was caught in review. Enumerate a passage's propositions before trimming it.
...
**Original:** "The gate notice narrates the check order; the notice text is also pinned by the diagnostics test."

**Overcorrected:** "…" (whole sentence deleted as narration.)

**Right:** "The notice text is pinned by the diagnostics test."
