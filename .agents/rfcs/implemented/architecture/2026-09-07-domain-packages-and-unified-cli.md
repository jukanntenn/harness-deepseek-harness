...
- The domain packages are `pairing/`, `docs/`, `rfc/`, `scope.py`, `policy/`, and `worktree/`; inside them modules split by concept — `pairing/{manifest,corpus,records,links,structure,git,verify,merge}`, `policy/{config,rules,client,commands}`, `worktree/{git,ownership,config,install}`, `docs/{config,corpus,markdown,wrap,links,budgets}`, `adopt/{corpus,manifest,commands}`.
...
- Gate configuration files mirror the domains — `.hdsh/pairing.manifest.json` and `.hdsh/docs.manifest.json` — and the merge driver is `merge.hdsh-pairing` backed by the `hdsh pairing merge-driver` CLI entry; the worktree installer registers both.
