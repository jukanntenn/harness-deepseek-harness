...
- **Issue references** — `#1470`, `TODO(name):`, "issue #N owns the follow-up" resolve at HEAD; keep them on any surface, including READMEs. Do not relocate them to RFCs.
- **Merged-PR and issue citations inside RFCs** — sanctioned evidence per [the documentation standard](../../../docs/AGENTS.md)'s writing rules, which route change stories to RFCs and commits.
- **Suppression justifications** — `# ruff: noqa` and `# pragma: no cover` reasons, empty-`except` explanations are required prose; fix a false reason, never delete it.
...
- **Historical stage names inside an RFC's change-story sections** — "the first cut shipped X" is current-state-safe there; indexical stamps ("this cut") stay banned everywhere.
- **External references that resolve outside the repo by design** — standards sections (RFC 9110 §10.1.5); the §-ban covers uncommitted internal drafts, not external standards or committed docs that own their §-numbering.
- **Project voice and genre forms** — "we" as project voice; an RFC's Alternatives-considered section.

## Workflow

1. Scope and exclusions per [editing-prose](../editing-prose/SKILL.md): require an explicit scope; never touch dependency, cache, build-output, or virtual-environment trees, and never touch `.agents/rfcs/archived/`. Recorded fixtures and snapshots are derivatives, not prose targets: change the owning source or scenario and regenerate them only when an authorized behavior change requires new evidence.
2. Audit read-only first: run the [recall batteries](references/recall-batteries.md) (with `--hidden` so `.agents/` is searched), calibrating each probe against a known positive and a near-miss negative before trusting its output, then judge every hit semantically. The batteries are probes, not the definition — also read the densest prose in scope (module docstrings, READMEs, RFCs) without a pattern in hand.
3. Fix owner-first per surface: generated regions → trace every consumer, fix the owning source, then update every derivative; bilingual prose → update the counterpart minimally and re-record it through the [routine path](../../../docs/AGENTS.md#writing-rules); bilingual fences → copy the corrected verbatim block byte-for-byte into both sides per [documenting](../documenting/SKILL.md), then re-record the pair; diagnostics and CLI strings → route through [editing-prose](../editing-prose/SKILL.md) and change only with owning behavior evidence, otherwise leave unchanged and report the deferral.
4. Before deleting anything, enumerate the passage's propositions (editing-prose) and check the [overcorrection traps](references/examples.md#overcorrection-traps): trims that flip an obligation into an endorsement, promote a hypothetical to a shipped feature, delete a true fact, or drop provenance.
5. Verify: re-run the batteries expecting only sanctioned keeps and this skill's own directory; confirm every remaining citation resolves at HEAD; run the gates for touched surfaces (`uv run hdsh docs wrap` and `uv run hdsh docs links` for docs, `uv run hdsh pairing verify` for pairs).
