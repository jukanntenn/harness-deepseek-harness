...
## Docstrings preserve boundary timing
...
**Over-detailed:** A walkthrough of the loop helpers, cache fields, and callbacks that implement the ordering.

Boundary order and its current-request consequence are caller-visible behavior, not implementation narration.
...
## Public docstrings include failures

**Over-trimmed:** "Returns the realm global."

**Balanced:** "Returns the initialized realm global. Raises RuntimeError if initialization has not completed or the realm has already been disposed."

**Over-detailed:** The internal state-machine branches and exact helper calls that lead to each raise.

Raised exceptions and state preconditions are caller-visible contract facts.
...
**Balanced:** "Mounted plugins share the host's authority; for example, access to the host shell permits commands with the host executor's privileges."
...
## Diagnostics and CLI text follow ownership

**Over-trimmed:** "The command reports errors when a gate fails."

**Over-detailed:** Copying another module's diagnostic strings into this module's README.

**Balanced:** Quote stable diagnostic and error text owned by this module. Link the owning reference for text another module owns; state only this module's conditions or deltas locally.

Wording that reaches a user is behavior, but duplication still drifts. Exactness belongs at the owner.

A prose-only audit may identify suspect wording but must not silently change it when no owning test pins the behavior. Leave it unchanged and report the deferral, or expand the authorized change to include the owning test and its behavior evidence.

## Limitations are contracts, not debt inventories

**Over-trimmed:** Omitting a process-lifetime cache that makes configuration changes require a reload.

**Over-detailed:** Listing private helper cleanup and unused test-only accessors with no caller or maintainer consequence.

**Balanced:** "Provider selection is cached for the process lifetime; installing or repairing a provider requires a reload." Keep ordinary cleanup in its TODO or RFC.
